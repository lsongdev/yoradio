...
void DspCore::printPLitemCHS(uint8_t pos, const char *item, ScrollWidgetCHS &current)
{
  setTextSize(playlistConf.widget.textsize);
  if (pos == plCurrentPos)
  {
    fillRect(0, plYStart + pos * plItemHeight - 1, width(), plItemHeight + 1, config.theme.background);
  ...
  else
  {
    ...
    u8g2_for_adafruit_gfx.drawUTF8(TFT_FRAMEWDT, plYStart + pos * plItemHeight + 16, item);
...
void DspCore::_clockTime()
{
  if (_oldtimeleft > 0 && !CLOCKFONT_MONO)
    dsp.fillRect(_oldtimeleft, clockTop - clockTimeHeight + 1, _oldtimewidth, clockTimeHeight, config.theme.background);
...
void DspCore::printClock(uint16_t top, uint16_t rightspace, uint16_t timeheight, bool redraw)
{
  ...
  if (strcmp(_oldTimeBuf, _timeBuf) != 0 || redraw)
  {
    _getTimeBounds();
    _clockTime();
    if (strcmp(_oldDateBuf, _dateBuf) != 0 || redraw)
      _clockDate();
  }
  _clockSeconds();
}
